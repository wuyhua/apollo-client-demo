package com.example.study.dubboExample.sequence;
...
public final class Sequence {
    ...
    public static void main(String[] args) {
        ...
        final Set<Long> objects = Sets.newConcurrentHashSet();

        for (int i = 0 ; i< threadNum ; i ++){

            new Thread(new GenIdTask(i,objects,latch)).start();
        ...
        System.out.println("id : " + objects.toArray()[0]);
        System.out.println("workId : " + (0x000000FF & getLastIPAddress()));

        System.out.println(threadNum* 1000000 == objects.size());


    }

    public static class GenIdTask implements Runnable{

        private int workId;

        private int count = 1;

        private Set<Long> collectSet;

        private CountDownLatch latch;

        public GenIdTask(int workId,Set<Long> collectSet,CountDownLatch latch) {
            this.workId = workId;
        ...
        @Override
        public void run() {
            Sequence sequence = new Sequence(workId);
            while (count > 0){
                try {
                    collectSet.add(sequence.nextId());
                }catch (Exception e){}
                count--;
            }
            latch.countDown();
